package me.lemon.addon.modules;

import me.lemon.addon.LemonClient;
import me.lemon.addon.utils.player.LPlayerUtils;
...
public class SanWuYiFly extends Module {
    public enum Type {
        Factor,
        SetBack,
        Fast,
    ...
    public SanWuYiFly() {
        super(LemonClient.Main, "SanWuYi-Fly", "Reminds of the wakanda airforce.");
    }
    ...
    public void onPostTick(TickEvent.Post event) {
        ...
        if ((multiAxis.get() && phasing) || !(mc.options.sneakKey.isPressed() && mc.options.jumpKey.isPressed())) {
            if (isPlayerMoving()) {
                double[] dir = directionSpeed((((phasing && phase.get() == Phase.NoCheatPlus ) || bypass.get() == Bypass.NoCheatPlus) ? (noPhaseSlow.get() ? (multiAxis.get() ? 0.0465 : 0.062) : 0.031) : 0.26) * speed.get());
        ...
        if (((phasing && phase.get() == Phase.NoCheatPlus) || bypass.get() == Bypass.NoCheatPlus) && (double) mc.player.forwardSpeed != 0.0 || (double) mc.player.sidewaysSpeed != 0.0 && speedY != 0) {
            speedY /= 2.5;
        }
        ...
        switch (type.get()) {
            ...
            case OnGround -> {
                ...
                for (double i = 0.0625; i < speed.get(); i += 0.262) {
                    double[] dir = LPlayerUtils.directionSpeed((float) i);
    ...
    public void onReceivePacket(PacketEvent.Receive event) {
        if (type.get() == Type.AutoElytra) return;
    ...
    public void onSend(PacketEvent.Send event) {
        if (type.get() == Type.AutoElytra && event.packet instanceof PlayerMoveC2SPacket) {
            mc.player.networkHandler.sendPacket(new ClientCommandC2SPacket(mc.player, ClientCommandC2SPacket.Mode.START_FALL_FLYING));
            return;
        }
    ...
    public void updateFlying(Type type) {
        if (mc.world != null && mc.player != null && type != Type.AutoElytra) {
            mc.player.getAbilities().flying = false;
            mc.player.getAbilities().allowFlying = false;
        }
    }
