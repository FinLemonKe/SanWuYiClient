...
public class ChatConfig extends Module {
    ...
    public final Setting<String> text = sgGeneral.add(new StringSetting.Builder().name("text").description("Text of the prefix").defaultValue("SanWuYiClient").build());
    ...
    public ChatConfig() {
        super(LemonClient.Main, "chat-config", "The way to render chat messages.");
    }

    @Override
    public void onActivate() {
        if (mode.get() == Mode.Lemon) ChatUtils.registerCustomPrefix("me.lemon.addon", this::getPrefix);
    }
